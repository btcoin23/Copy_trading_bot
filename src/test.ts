...
const logger = initLogger("transaction-logs.txt");
  ...
  const anaylzeSignature = async (log: any) => {
    try {
      ...
      if (
        ...
        if (!txnData) throw new Error("Txn data not found");
        ...
        console.log(
          `${
            txnData.is_buy ? "🟢 Bought" : "🔴 Sold"
          } | ${signature} from wallet: ${key} | Token: ${
            txnData.mint
          } | Amount: ${txnData.delta_token.toFixed(
            3
          )} | SOL: ${delta_sol.toFixed(
            9
